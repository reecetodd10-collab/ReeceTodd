...
export default function Settings() {
  ...
  const [testingMode, setTestingMode] = useState(TESTING_MODE);
  ...
  useEffect(() => {
    ...
    const goalSettings = gamificationData.dailyGoalsSettings || {
      trackWater: true,
      trackSleep: true,
      trackMeals: true,
      trackMacros: true,
  ...
  return (
    <div>
      ...
      {TESTING_MODE && (
        <div className="glass-card p-6 mb-6">
          <div className="flex items-center gap-3 mb-4">
            <h2 className="text-xl font-bold text-[var(--txt)]">Development</h2>
          </div>
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium mb-1 text-[var(--txt)]">Testing Mode</p>
              <p className="text-[var(--txt-muted)] text-sm">Enable testing features</p>
            </div>
            <button
              onClick={() => setTestingMode(!testingMode)}
              className={`
                w-12 h-6 rounded-full transition
                ${testingMode ? 'bg-[var(--acc)]' : 'bg-[var(--bg-elev-1)]'}
              `}
            >
              <div
                className={`
                  w-5 h-5 bg-white rounded-full transition-transform
                  ${testingMode ? 'translate-x-6' : 'translate-x-0.5'}
                `}
                style={{ marginTop: '2px' }}
              />
            </button>
