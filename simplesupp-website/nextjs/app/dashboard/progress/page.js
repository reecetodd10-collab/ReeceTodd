...
export default function Progress() {
  ...
  const getMonthlyStats = () => {
    if (!data) return { completed: 0, totalXP: 0, averageXP: 0, totalDays: 0 };
    const today = new Date();
    ...
    <div>
      ...
      <GlassCard className="p-6 mb-6">
        ...
        <div className="flex items-center justify-between mt-4 text-xs text-[var(--txt-muted)]">
          ...
          <span>{heatmapDays.length > 0 && `${new Date(heatmapDays[0].date).toLocaleDateString()} - ${new Date(heatmapDays[heatmapDays.length - 1].date).toLocaleDateString()}`}</span>
        </div>
      </GlassCard>
