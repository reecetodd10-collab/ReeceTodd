    ...
    const newWeek = {
      ...
      days: currentWeek.days.map(day => ({
        ...
        exercises: day.exercises.map(ex => ({
          ...
          weight: ex.weight ? Math.round(ex.weight * 1.05) : null, // 5% increase (progressive overload)
          completedSets: Array(ex.sets).fill(false),
          notes: ex.notes,
        })),
      })),
...
function ExerciseCard({ exercise, dayIndex, exerciseIndex, onEdit, onRemove, onSwap, onUpdate, onDragStart, onDragOver, onDragEnd, onStartRestTimer, parseRestTime }) {
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          {preview && preview.length > 0 ? (
            ...
            <p className="text-sm text-[var(--txt-muted)]">No weight increases detected. Progressive overload will apply 5% increases.</p>
          )}
        </div>
        <div className="flex gap-3">
          <Button variant="secondary" onClick={onClose} className="flex-1">Cancel</Button>
